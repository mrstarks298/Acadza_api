...
function generateHTML(input) {
  ...
  <head>
    ...
    <style>
      ...
      .query-block { font-size: 13px; margin: 4px 0 12px; }
      .section-label { font-size: 14px; font-weight: bold; margin: 10px 0 8px; }
      h1 { font-size: 18px; border-bottom: 1px solid #666; padding-bottom: 4px; margin-top: 16px; }
      h2 { font-size: 15px; margin-top: 14px; color: #003366; }
      p { font-size: 13px; margin: 6px 0; }
      ul { font-size: 13px; margin-left: 18px; padding-left: 10px; list-style-type: disc; }
      .callout { background-color: #eef7ff; border-left: 4px solid #1e90ff; padding: 8px; margin: 12px 0; }
      .quote { font-style: italic; font-size: 12px; margin-top: 14px; color: #555; }
      .quote-author { text-align: right; font-size: 11px; margin-top: 3px; }
      .test-box { border: 1.5px solid orange; padding: 14px; margin-top: 20px; background: #fff5e6; border-radius: 4px; }
      .test-link { display: inline-block; background: rgb(233, 102, 16); color: white; padding: 8px 14px; text-decoration: none; border-radius: 3px; font-weight: bold; margin-top: 10px; }
      .report-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
      .logo-line { display: flex; align-items: center; }
      .logo { height: 28px; margin-right: 8px; border-radius: 4px; background-color: white; padding: 2px; }
      .institution { font-weight: bold; font-size: 14px; color: black; }
      .timestamp { font-size: 12px; color: #555; }
      .header-divider { border: none; border-bottom: 1px solid #ccc; margin-bottom: 16px; }
...
`;
  ...
  general_script.forEach(block => {
    ...
    if (block.callout) html += `<div class="callout">${block.callout.content}</div>`;
    if (block.quote) {
  ...
  html += `<div class="footer">© 2025 Acadza Technologies — Auto-generated report</div></body></html>`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!input) return res.status(400).json({ error: 'No input data provided' });
    if (!validateInput(input)) return res.status(400).json({ error: 'Invalid input.json format' });
